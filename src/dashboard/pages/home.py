...
min_simulation_date = pd.to_datetime(cfd_df["date"]).min()
max_simulation_date = pd.to_datetime(cfd_df["date"]).max()
...
with tgb.Page() as homepage:
    ...
    with tgb.part(class_name="container"):
        with tgb.part(class_name="card"):
            tgb.text("## Data Set Information",
                     mode="md",
                     class_name="h3 text-center pb1"
                     )
        ...
        with tgb.layout("1 1 1"):
            with tgb.part(class_name="card"):
                tgb.text("## Number of Simulations: {num_simulations}",
                         mode="md",
                         class_name="h4 text-center pb2",
                         )

            with tgb.part(class_name="card"):
                tgb.text("## Shapes: {unique_shape_count}",
                         mode="md",
                         class_name="h4 text-center pb2",
                         )
    ...
    with tgb.part(class_name="container", columns="1 1 1"):
        with tgb.layout("1 1 1"):
            with tgb.part(class_name="card"):
                tgb.text("## Designs: {unique_design_count}",
                         mode="md",
                         class_name="h4 text-center pb1",
                         )
